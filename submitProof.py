...
def merkle_assignment():
    ...
    random_leaf_index = None
    indices = list(range(1, num_of_primes))
    random.shuffle(indices)
    for idx in indices:
        owner = contract.functions.getOwnerByPrime(primes[idx]).call()
        if owner == '0x0000000000000000000000000000000000000000':
            random_leaf_index = idx
            print(f"Found unclaimed prime: {primes[idx]} at index {idx}")
            break

    if random_leaf_index is None:
        print("No unclaimed primes found!")
        return
...
def generate_primes(num_primes):
    ...
    if num_primes < 6:
        ...
        upper = int(num_primes * (math.log(num_primes) + math.log(math.log(num_primes)))) + 3
...
def prove_merkle(merkle_tree, random_indx):
    ...
    for level in range(len(merkle_tree) - 1):
        ...
        # Handle odd-length levels same way as build_merkle
        if len(current_level) % 2 == 1:
            current_level = current_level + [current_level[-1]]
